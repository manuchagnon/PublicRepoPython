...
def create_flc_on_surface(
    ...
    I suggest to use auto_rig.component.ribbon and distribute_follicles_on_surface to be more flexible
    """
    ...
    flc_created = []
    # flc_grp = cmds.group(name=f'{suffix}{srf_name}_grp', empty=1)
    ...
    for i_u in range(flc_on_u):
        for i_v in range(flc_on_v):
            ...
            flc_shape = cmds.createNode("follicle", name=f"{suffix}{srf_name}_{flc_index}Shape")
            cmds.setAttr(f"{flc_shape}.visibility", 0)
    
            flc_transform = cmds.listRelatives(flc_shape, parent=1)[0]
            cmds.rename(flc_transform, f"{suffix}{srf_name}_{flc_index}")
            flc_transform = f"{suffix}{srf_name}_{flc_index}"
